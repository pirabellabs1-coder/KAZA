...
export default async function AgencyReviewsPage() {
  ...
  // owner_id des biens = id du compte agence : on réutilise la requête owner.
  const received = await listOwnerReceivedReviews(user.id);

  const reviews: ReviewItem[] = received.map((r) => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Avis laissés par les locataires et visiteurs sur les biens de votre
          agence. Soignez votre réputation pour attirer plus de mandats.
        </p>
      </div>
      ...
      {total === 0 ? (
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-border bg-muted/30 p-10 text-center">
          ...
          <p className="max-w-md text-sm text-muted-foreground">
            Une fois les locations terminées, les locataires pourront laisser un
            avis. Les évaluations de votre agence s&apos;afficheront ici.
          </p>
        </div>
      ) : (
        <>
